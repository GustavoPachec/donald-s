  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center gap-3">
        <ScrollTextIcon />
        <h2 className="text-lg font-semibold">Meu Pedidos</h2>
      </div>
      {orders.map((order) => (
        <Card key={order.id}>
          <CardContent className="space-y-4 p-5">
           ...
           <div className="space-y-2">
             {order.orderProducts.map(orderPoduct => (
              <div key={orderPoduct.id} className="flex items-center gap-2">
                <div className="h-5 w-5 flex items-center justify-center rounded-full bg-gray-400 text-white text-xs font-semibold">
                  {orderPoduct.quantity}
                </div>
                <p className="text-sm">{orderPoduct.product.name}</p>
              </div>
            ))}
           </div>
            <Separator />
